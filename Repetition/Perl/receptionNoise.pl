...
# on test la paritee de cette reception (on test si la reception est valide) :
   $nbrErrD+=1 unless (Repetition::verification($carRec)); # la reception ne respecte pas la parite

# on test la validite de ce caractere recu :
   $nbrErr+=1 if ( Repetition::decodage($carRec) ne 'o'); # le caractere n'est pas celui attendu
